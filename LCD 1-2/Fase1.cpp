...
FT_HANDLE& deviceHandler_lcdInit(int iDevice) //Inicializacion del modo 4 bits.
{
	...
	if (FT_Open(iDevice, &deviceHandler) == FT_OK)
	{
		...
		if (FT_SetBitMode(deviceHandler, Mask, Mode) == FT_OK)	
		{
			...
			lcdWriteByte(&deviceHandler, 0x38, IR);						//7. Enviar la instrucción “function set” con el modo en 4 bits, 2 líneas y fuente de 5x8
			lcdWriteByte(&deviceHandler, 0x08, IR);						//8. Enviar la instrucción “display on / off control” con todo apagado
			lcdWriteByte(&deviceHandler, 0x01, IR);						//9. Enviar la instrucción “clear screen”
			lcdWriteByte(&deviceHandler, 0x06, IR);						//10. Enviar la instrucción “entry mode set”
